...
namespace ActivityTracer.Areas.Identity.Pages.Account
{
	[AllowAnonymous]
		...
		public async Task<IActionResult> OnGetCallbackAsync(string returnUrl = null, string remoteError = null)
		{
			...
			else
			{
				...
				if (info.Principal.HasClaim(c => c.Type == ClaimTypes.Email))
				{
					...
					var access_token_json = new WebClient().DownloadString("https://graph.facebook.com/oauth/access_token?client_id=950452413482166&client_secret=427384153dc998e129210735438c2faa&grant_type=client_credentials");
					var token = JsonConvert.DeserializeObject<TokenModel>(access_token_json);
					Input.PictureUrl = $"https://graph.facebook.com/{info.Principal.FindFirstValue(ClaimTypes.NameIdentifier)}/picture?type=large&access_token={token.access_token}";
				}
				return Page();
			}
		}
